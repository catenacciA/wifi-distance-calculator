...
void Logger::logData(const std::string &cellId,
  ...
  if (!fileExists) {
    logFile << "CellIDx,CellIDy,BSSID,AvgRSSI,StdDevRSSI,MinRSSI,MaxRSSI,Distance" << std::endl;
  }

  for (const auto &[bssid, stats] : rssiStats) {
    auto [mean, stdev, min, max, distance] = stats;
    logFile << cellId << "," << bssid << "," << mean << "," << stdev << "," << min << "," << max << "," << distance << std::endl;
  }
  logFile.close();
}
