...
double DistanceCalculator::calculateDistance(double rssi, double referenceRssi, double pathLossExponent, double referenceDistance) {
    return referenceDistance * std::pow(10.0, (rssi - referenceRssi) / (10.0 * pathLossExponent));
}

// Example function that uses AccessPointInfo
double DistanceCalculator::calculateDistanceToAP(const WiFiScanner::AccessPointInfo& ap) {
    ...
    return DistanceCalculator::calculateDistance(ap.signalStrengthDbm, referenceRssi, pathLossExponent, referenceDistance);
}
